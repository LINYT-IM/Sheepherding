...
showTyp = ''
try:
    opts, args = getopt.getopt(sys.argv[1:],"hn:i:d:m:s:",["sheep=", "index=", "drone=","method=","showType="])
...
if typ != 'm3':
    midFilename = ''
    if typ == 'S1':
        midFilename = 'input/N_results_100/input_point_'+str(n)+'_'+str(f_ind)+'.txt'
    else:
        midFilename = 'input/k_results_100/k_input_point_'+str(n)+'_'+str(f_ind)+'.txt'
...
optFilename = typ + '_case_' + str(n) + '_' + str(f_ind) + '_' + str(m)  + '.mp4'
path = readfile(moveFilename, n, m)
colmap = {}
if typ == 'S2' or typ == 'S3':
    col_size = len(g)+1
else:
