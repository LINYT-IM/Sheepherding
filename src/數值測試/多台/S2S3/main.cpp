...
int main(){
    ...
    for(int input_n = 5 ; input_n<=20 ; input_n+=5){
        for(int input_index =0 ; input_index<30; input_index++){
            // string filename_move  = "../../movement/S2_case_" + ::to_string(input_n)+"_"+std::to_string(input_index) + "_" + ::to_string(m_of_D)+"_"+std::to_string(g)+"_Mu.txt";
            string filename_move  = "../../movement/S3_case_" + ::to_string(input_n)+"_"+std::to_string(input_index) + "_" + ::to_string(m_of_D)+"_Mu.txt";
            // string filename_midP  = "../../midP/S2_case_" + ::to_string(input_n)+"_"+std::to_string(input_index)+ "_" +::to_string(m_of_D)+"_"+std::to_string(g)+"_Mu.txt";
            // createFile(filename_move);
            // createFile(filename_midP);
            string filename = "../../input/results_newS3_123/k_input_point_"+std::to_string(input_n)+"_"+std::to_string(input_index)+"_"+std::to_string(g)+".txt";
            // string filename = "../../input/results_211208/k_input_point_"+std::to_string(input_n)+"_"+std::to_string(input_index)+".txt";
            ...
            for(int iteration = 0 ; iteration<iteration_time ; iteration++){
                ...
                pair<float, float> *total_meet;
                float max_x=0, max_y=0, tar_x, tar_y;
                vector<int> group_list;
                vector<int> *paths;
                
                pair <float, float> target(0,0);
                readfile(&read, filename, &n_of_s, &max_n, &m, nodeList, group_m, meet, paths, group_list, &max_x, &max_y, &tar_x, &tar_y);
                int total_p = n_of_s + m + max_n ;//中繼點＋羊群數量
                total_meet = new pair<float, float>[max_n+m];
                for(int i=0; i<max_n; i++){
                    total_meet[i] = group_m[i];
                }
                for(int i=max_n; i<max_n+m; i++){
                    total_meet[i] = meet[i-max_n];
                }
                // cout << total_p << endl;
                // 建立羊群物件
                Sheep *sheeps = new Sheep[n_of_s];
                ...
                int count = 0;
                vector<int> pre_group[m+max_n+1];
                int keys[total_p+1];
                for(auto it = group_list.begin(); it != group_list.end(); ++it){
                    pre_group[*it].push_back(count);
                    keys[count] = *it+n_of_s;
                    count += 1;
                }
                // for(int i = 0; i < max_n+m; i++){
                //     cout << "mid point " << i+n_of_s << " : (" << total_meet[i].first << ", " << total_meet[i].second << ")\n";
                // }
                // for(int i=0; i<max_n; i++){
                //     cout << "path " << i << " : ";
                //     for(auto it = paths[i].begin(); it != paths[i].end()-1; ++it){
                ...
                for(int i=0; i<max_n; i++){
                    for(auto it = paths[i].begin(); it != paths[i].end()-1; ++it){
                        keys[*(it)] = *(it+1);
                        if(*(it+1) != -1){
                            pre_group[*(it+1)-n_of_s].push_back(*(it));
                            for(auto it2 = paths[i].begin(); it2 != it+1; ++it2){
                                for(auto it3 = pre_group[*it2-n_of_s].begin(); it3 != pre_group[*it2-n_of_s].end(); ++it3){
                                    if (find(pre_group[*(it+1)-n_of_s].begin(), pre_group[*(it+1)-n_of_s].end(), *(it3)) == pre_group[*(it+1)-n_of_s].end()){
                                        pre_group[*(it+1)-n_of_s].push_back(*(it3));
                                    } 
                                }
                            }
                        }else{
                            pre_group[m+max_n].push_back(*(it));
                            for(auto it2 = paths[i].begin(); it2 != it+1; ++it2){
                                for(auto it3 = pre_group[*it2-n_of_s].begin(); it3 != pre_group[*it2-n_of_s].end(); ++it3){
                                    if (find(pre_group[m+max_n].begin(), pre_group[m+max_n].end(), *(it3)) == pre_group[m+max_n].end()){
                                        pre_group[m+max_n].push_back(*(it3));
                                    } 
                                }
                            }
                        }
                ...
                // for(int i = 0; i < max_n+m+1; i++){
                //     cout << "PreGroup of mid point " << i+n_of_s << " : ";
                ...
                while(undone.size() > 0 && !gameover){
                    ...
                    for(int i = 0; i < m_of_D; i++){
                        ...
                        if(player[i].idle){
                            curr_node = findNearest(sheeps, total_meet, undone, player[i].pos, n_of_s, total_p, n_ava);
                        }else{
                    ...
                    if(!conti){
                        ...
                        for(int i = 0; i < group_n; i++){
                            ...
                            if(curr[i] != -1){
                                if(curr[i] < n_of_s){
                                    ...
                                    for(auto it = pre_group[curr[i]-n_of_s].begin(); it != pre_group[curr[i]-n_of_s].end(); ++it){
                                        if(*it < n_of_s){
                                ...
                                if(next_node != -1){
                                    goal[i] = pair<float, float>(total_meet[next_node-n_of_s].first, total_meet[next_node-n_of_s].second);
                                }else{
                                    goal[i] = target;
                                }
                            }
                            ...
                            //     cout << *it << "\t";
                            // }
                            // cout << " ], goal is  (" << goal[i].first << ", " << goal[i].second << "), pass time(prev): " << pass_time << endl;

                        }
                        pass_time = black_box(player, m_of_D, sheeps, n_of_s, movesheep, group_n, total_meet, m, goal, limit, target, filename_move);
                        ...
                        for(int i = 0; i < max_n+m; i++){
                            if(find(done.begin(), done.end(), i+n_of_s) == done.end() && find(undone.begin(), undone.end(), i+n_of_s) ==undone.end()){
                                bool flag = true;
                                for(auto it = pre_group[i].begin(); it != pre_group[i].end(); ++it){
