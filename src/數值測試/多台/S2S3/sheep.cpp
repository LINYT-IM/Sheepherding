...
int Sheep::getcurrPos(){
    return this->currPos;
}

void Sheep::setcurrPos(int pos){
    this->currPos = pos;
}
...
void sheepMover(Sheep* sheeps, Player *player, int n, int limit, vector<int> movesheep, pair<float, float> goal){
    for(int i = 0; i < n; i++){
        if (find(movesheep.begin(), movesheep.end(), i)  != movesheep.end()){
            switch(sheeps[i].getSta()){
                ...
                case 1: 
                    ...
                    break;
                case 2: 
                    float dis = sqrt(pow(sheeps[i].getPos().first- goal.first,2) + pow(sheeps[i].getPos().second- goal.second,2));
                    if (dis >= 2){
                        sheeps[i].randMove(limit);
                    }
                    break;
...
bool checkStatus(Sheep *sheeps, vector<int> ava, int n){
    for(int i = 0; i < n; i++){
